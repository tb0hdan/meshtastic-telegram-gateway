...
class RichConnection(MeshtasticConnection):
    ...
    def nodes_with_position(self) -> List:
        ...
        for node_info in self.nodes_with_info:
            ...
            lat_r = self.config.enforce_type(float, self.config.WebApp.Center_Latitude) +  random.randrange(1000) / 10000
            lon_r = self.config.enforce_type(float, self.config.WebApp.Center_Longitude) +  random.randrange(1000) / 10000
            position = node_info.get('position', {})
            if not (position.get('latitude') and position.get('longitude')):
                self.logger.debug(f"Node {node_id} doesn't have position...")
                try:
                    lat, lon = self.db.get_last_coordinates(node_id)
                except RuntimeError:
                    pass
                lat = lat if lat else lat_r
                lon = lon if lon else lon_r
                latitudeI = str(lat).replace('.', '')[:9]
                longitudeI = str(lon).replace('.', '')[:9]
                node_info['position'] = {'latitude': lat, 'longitude': lon,
                                         'latitudeI': latitudeI, 'longitudeI': longitudeI,
                                         'altitude': 100}
